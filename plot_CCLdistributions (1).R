...
all.data <- rbind(data.hoyt.CCL,
  ...
  group_by(Source) %>%
  mutate(SourceNum = ifelse(Source == 'Peckham', 1,
                            ifelse(Source == 'Turner_Tomaszewicz', 2,
                                   ifelse(Source == 'Fishery', 3, 4))))

# Do "anova" here: https://darrenjw.wordpress.com/2014/12/22/one-way-anova-with-fixed-and-random-effects-from-a-bayesian-perspective/
############################################################################
library(rjags)
data <- list(y=all.data$CCL,g = all.data$SourceNum, 
             N = nrow(all.data), p = 4)
...
# or just a glm
lm.source <- lm(CCL ~ Source, data = all.data)
sum.lm <- summary(lm.source)
##

# either way, the means are different. 
...
if (save.fig){
  dpi <- 300
  ggsave(plot = p2,
         filename = paste0('figures/CCL_all_sources_', dpi, 'dpi.png'),
         dpi = dpi)
